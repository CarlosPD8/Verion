...
System computes priority and confidence for each Risk using an explainable combination of: severity, confidence, exposure, ~~reachability (where available), asset sensitivity, and environment~~. Every score must be traceable to its inputs — no unexplained single number. *(**Cut 2026-09-19, M7→M8 boundary review: reachability, asset sensitivity and environment move to V2**, on ADR-0005 decision 4's ground that none has a source in `src/`. The Risk's confidence stays, and is `ROADMAP.md` M8.5.)* *(**Decided 2026-09-20, M8.5, ADR-0037; met when that issue's code commit lands.** A Risk is to carry a confidence, returned by every Risk-bearing response — but it is the Risk's **grouping provenance**, whether the surface's membership came off fields the scanners reported or was partly inferred from a route map, and it is **not computed from the `confidence` input this sentence names**. That input is a per-finding confidence, which only ZAP supplies and which ADR-0005 decision 4 declined; its re-propose condition is unmet and **G94** carries it. So the **output** half of this requirement is met and the **input** half is not. The confidence is carried beside the priority and is never summed into it, so "traceable to its inputs" is unaffected.)*

**FR-8 — Security Brief**
For each prioritized Risk, system generates a structured explanation: what happened, why it matters, evidence sources, ~~recommended action, estimated effort (qualitative),~~ confidence. *(**Cut 2026-09-19, M7→M8 boundary review: recommended action and estimated effort move to V2.** Remediation needs typed fields in `normalization`, a migration and a change to ADR-0019's refresh set (ADR-0034 decision 1), and effort has no deterministic input. A Brief has four parts: what happened, why it matters, evidence sources, confidence.)* *(**Decided 2026-09-20, M8.5, ADR-0037; met in all four parts when that issue's code commit lands.** Confidence was the last one outstanding and ships on both Brief routes as `{value, definition}`. It is the Risk's grouping provenance — see FR-7's note above for what it is and is not — and the definition rides the response because no prompt receives the value, so nothing but that text and the field name carries its meaning to a reader. A Brief written before M8.5 returns `null` for it and is not backfilled.)*
...
5. **Everything auditable.** Every risk state change (opened, dismissed, resolved) is logged with actor and timestamp. *(**Qualified 2026-09-19, M8.1, ADR-0036:** dismissal and its undo are logged, append-only, with actor, reason and time. **"Opened" is not logged:** a candidate Risk is recomputed per request (ADR-0025 decision 1), so it has no moment of creation for anything to record. A Risk that reopens because a new finding joined it is derived at read time and is not logged either. Resolution, and recording detected changes, are `ROADMAP.md` M9.1's.)*
6. **Dogfooding.** Verion should scan its own repository as part of CI, using itself as the first proof point.
7. **Inbound events are verified before they are trusted.** Any endpoint accepting provider-originated events (GitHub push/PR today) verifies the payload signature over the *raw* request body before any use case runs, and deduplicates redeliveries. An unauthenticated endpoint that triggers real work — cloning a repo, running scanners — is otherwise a spoofing and free-work surface. See ADR-014. *(Appended as 7 rather than inserted: `§11.1` and `§11.5` are cited by ADR-011 and ADR-014, so renumbering would break them.)* *(Corrected 2026-09-17, M7.3: that list was stale. Measured with `git grep -o -E "§ ?11\.[0-9]"`, the cited items are §11.1 (ADR-0011, ADR-0024, `ROADMAP.md`), §11.2 (ADR-0019, ADR-0022, `ROADMAP.md`), §11.3 (`ROADMAP.md`), §11.5 (ADR-0014, ADR-0019, ADR-0024, `ARCHITECTURE.md`, `ROADMAP.md`, `handle_github_webhook.py`) and §11.6 (ADR-0019). The rule is unchanged: append, never insert.)*
8. **Scanned content is untrusted input to the LLM.** Text originating in a scanned repository reaches a model only as bounded, character-sanitized, structurally encoded data, in a prompt that decides nothing; the prompt that narrates a priority carries none of it; and model output built from it is validated before it is stored, never rejecting on text the scanned content itself supplied. See ADR-0034. *(Appended as 8, per the note above.)*
