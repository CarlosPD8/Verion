...
The timestamps are deliberately **not** constrained, in either place. `started_at`/`finished_at` belong to transitions M4.4 writes; pinning them now would be constraining a state machine this issue does not implement.
