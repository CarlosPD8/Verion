...
- **2026-08-27: decision 4's code block shows `async def run(self, target: str) -> RawScanResult: ...`, and that signature is superseded by ADR-0024 decision 4**, which adds a `ScanOptions` parameter so `ZapAdapter` can read active-scanning consent at its plan-building call site. Decision 4's own reasoning is unaffected and is what ADR-0024 cites as its precedent: dispatch routes on data the port carries, never on a `tool == "zap"` branch.

- **2026-08-27: decision 1's timeout arithmetic and Consequences' `~300s of wall clock` are superseded — ZAP's timeout is 540s since M5.4, not 300s.** ADR-0024 decision 5 required the change: an `activeScan` job pushes the plan's own ceilings past the old timeout, and the plan must sum below it or the caps stop binding. Decision 1's inputs become `ZAP 540s, Trivy 180s, Semgrep 60s`, so `max()` plus a 30s checkout is **570s** rather than ~300s, and the Consequences figure is 570s of wall clock. **The decision itself is untouched and the margin is now the point rather than an aside:** `job_timeout` stays 600s — raising it makes the reconciliation sweep re-enqueue live work (`platform/settings.py`'s comment on `normalization_sweep_stale_after_seconds`) — so 30s is all that separates a full three-scanner run from arq killing it, and a further scanner-timeout raise is no longer a local decision.

- **2026-09-15 (M5→M6 boundary review): decision 3's *"both of ADR-013's gates still run as the literal first lines of `ZapAdapter.run()`"* repeats wording ADR-0013 struck on 2026-08-27**, and was not corrected when that was. Read it as ADR-0013's restatement: both gates run before the target is contacted and before any subprocess is spawned, and neither runs when `allow_private_targets` is `True`. Decision 3's argument does not rest on "first lines" and stands.

- **2026-09-19 (M8.8): Consequences' *"Failures before any tool runs … still re-raise and still retry"* is false in the arq this project runs.** arq 0.28.0 retries a job only on `Retry`, `RetryJob` or `CancelledError`. The six pre-tool failures re-raise ordinary exceptions, so they finish the job as failed on the first attempt, and the scan stays `FAILED`. The trade that paragraph accepted, *"a transient scanner failure now needs a re-trigger"*, therefore holds for pre-tool failures too. Decisions 1 and 2 are unaffected: a retry, when one happens, still re-runs every enabled scanner. Recorded in ADR-0017's 2026-09-19 amendment and registered as **G87** (ADR-0035 decision 6).
