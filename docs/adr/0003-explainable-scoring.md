...
- **2026-09-16 (M6.1): the Decision's five-signal list is narrowed by measurement, and the constraint it exists to state is untouched.** ADR-0005 ships the function this ADR deferred, and it does not use three of the five signals named above.
  ...
  - **Nothing here is reversed.** The constraint — an inspectable function of explicit signals, a bucket, a confidence, a `RiskReasoning`, no trained model — is what ADR-0005 satisfies, and the signals above remain the target rather than becoming wrong. What this amendment records is that the list was written before any of its inputs had a source, and three of them still do not.
    - ***(Corrected 2026-09-17, M7.1, ADR-0032 decision 3.)*** *"a bucket, a confidence, a `RiskReasoning` … is what ADR-0005 satisfies"* is **false in its middle term and was false when written.** ADR-0005 decision 1 deferred the confidence scale, M6.2 emitted none, and M7.1 records the absence rather than choosing a scale, because either candidate reaches a third module. So shipped code satisfies the bucket and the `RiskReasoning`, and **not** the confidence this Decision names. The Decision is not struck: a confidence stays the target and unmet, and **G63** carries it.

- **2026-09-20 (M8.5, documentation commit): the Decision's "a confidence value" is DECIDED, and the 2026-09-17 correction above is discharged as to that clause when M8.5's code lands.** ADR-0037.
  - **What the correction said**, and it is still true at this commit: *"shipped code satisfies the bucket and the `RiskReasoning`, and **not** the confidence this Decision names."* ADR-0037 decides the third: `ScoredSurface` and `ExplainableRisk` are to carry a `Confidence`, and `/scored-risks` and the two Brief routes are to return it. **This commit writes no `src/`**, so the correction stands until M8.5's second commit; the discharge is recorded here because the decision is made here.
  - **Which confidence, stated because this Decision does not say.** It is the Risk's **grouping provenance** — whether the surface's membership came off fields the scanners reported or was partly inferred from a route map. It is not a confidence *about the evidence*, and it is computed from no per-finding confidence.
  - **So FR-7 is still not fully met, and this Decision is not the place that records it.** FR-7 asks for a confidence computed from an explainable combination of severity, **confidence** and exposure. ADR-0005 decision 4 declined `Finding.confidence` and its re-propose condition is unmet. **G94** carries that; **G63** resolves as to this ADR and as to FR-8, and not as to FR-7.
  - **The constraint this ADR exists to state is met, not merely satisfied in form.** The value is a documented function of explicit inputs — which branch of `build_match_key` placed each member — with no trained model and no hidden weight, and it is **not summed into the priority** (ADR-0037 decision 7), so the bucket stays re-derivable by hand from `RiskReasoning` exactly as before.
  - **The five-signal list is untouched** by this amendment; the 2026-09-16 amendment above still holds for it.
