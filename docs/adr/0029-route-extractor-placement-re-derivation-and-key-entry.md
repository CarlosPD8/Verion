...
- **2026-09-20 (M8.5, documentation commit): decision 4's third constraint gains a second half — the builder is to return the key's PROVENANCE beside the key. Decision 4 itself is unchanged.** ADR-0037 decision 4. **This commit writes no `src/`**; the change lands in M8.5's second commit.
  - **What changes.** `build_match_key` returns a `MatchKey`; it is to return that key and a `Confidence` naming which branch produced it — the url branch and the package branch `reported`, the derived-route branch `inferred`, no signal `ungrouped`. Decision 4's chosen shape 3 is untouched: the derived route path still goes into the existing `url` signal, ZAP is still keyed on path, and the derivation still happens at this site and never in a mapper.
  - **Why it is an amendment rather than a tidy-up, in the words that asked for one.** **G53**'s `Deferral rationale:` names this exact change as its closure — *"a provenance-aware check at the construction site — a second parameter naming where `url` came from, or a distinct type for a derived path — either of which is a design change to a site an accepted ADR describes, and therefore an amendment rather than a tidy-up."* This is that sentence being honoured.
  - **What it is to repair.** Decision 4's own stated price was that the site *"keeps type-checking exactly as before … while the provenance changes"* — ADR-0023 section (c)'s *"semantic changes behind an unchanged signature"*, entered deliberately. The provenance becomes a value the signature carries, so that particular blindness ends at this site. **G53 resolves when that commit lands**, and stays `assigned → M8.5` until it does.
  - **What it does NOT repair, and the residue is restated so it is not assumed closed.** The third constraint's other half stands: the conformance test still derives from the **key's** fields, so `file_path`, `start_line` and the `PathsServing` protocol still get `mypy` and no conformance assertion. A `Location.start_line` re-typed from `int | None` would still fail at the builder and nowhere else. That half moves to a note on G53 rather than closing with it.
  - **What is to be asserted instead of inferred.** Every branch's provenance is covered by a test that the values the builder can produce and the values the enum declares are the same set, so a fourth branch cannot be added without deciding its provenance and an enum member no branch reaches cannot be added either.
  - **Decisions 1, 2, 3 and 5 are unaffected.** In particular decision 3's third-tree residue is untouched: the provenance says a route map placed a member, never that the map describes the tree any scanner read (**G52**).
