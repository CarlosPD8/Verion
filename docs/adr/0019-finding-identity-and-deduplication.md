...
Measured cost on the real fixture: 4 findings → 11, and evidence bytes 8,523 → 18,148 (**2.13×**). *(**Pre-G23 figures.** Against the current corpus it is **5 alerts → 13 findings**; the byte figures were deliberately not re-derived — see Amendments below, which says so explicitly and asks that they not be quoted from this line. Marked inline at M5.1 because the identical claim in `ROADMAP.md` M4.2 got a marker when the corpus was replaced and this copy did not, which is the asymmetry **G20** exists to catch.)* See decision 5 for what that does to ADR-0018's storage estimate and why the character cap does not need revisiting.
...
- **2026-08-23 (M4.3):** **Decision 2's `check_id` caveat is corrected — it was measured, and it was measured incompletely.** It says the identifier "embeds the ruleset file's path", from which the stated consequence follows that *moving* `rulesets/default.yml` re-keys every Semgrep finding. The true property is broader: **`check_id` is the `--config` path rendered relative to the process's current working directory**, so the value changes when the working directory changes, not only when the file does. Measured across three CWDs with one identical absolute `--config` and one identical target: from the repo root `src.verion.modules.…rulesets.dangerous-eval`, from `<repo>/src` `verion.modules.…rulesets.dangerous-eval`, and from outside the repo the fully absolute `C.Users.…Verion.src.verion.modules.…rulesets.dangerous-eval`. `Settings.semgrep_ruleset` is already an absolute path, so `rule_id` — a decision 3 hash input — differs between a developer's machine, CI and a container, and the committed fixture can only ever show the first of the three.
  - **The sharp consequence, which the original caveat could not have anticipated: G9's own prescribed fix fires this.** Running `SemgrepAdapter` with `cwd=target` over a `tempfile.mkdtemp` puts the ruleset outside the working directory and flips `check_id` to the absolute form — so the change that stops `file_path` re-keying every Semgrep finding would re-key them all through `rule_id` instead, in the same commit.
  - Registered as **G10**, `Status: assigned → M4.4`, which is where G9's adapter change lands and where the first production writer arrives. **Nothing in decision 2's reasoning changes**: `rule_id` is still the tool's own stable identifier normalized by the mapper, the one-hash-function argument is untouched, and the mapper still records `check_id` verbatim. What changes is how *stable* that input is known to be, and therefore that a mitigation has to be chosen rather than assumed unnecessary. The obvious one — normalizing `check_id` to its last dotted segment — is ruled out in G10 on this ADR's own principle: registry rules carry meaningful namespaces, so it would collapse distinct rules into one and *fabricate* merges rather than under-count.
  - Decisions 1, 3, 4, 5, 6 and 7 are unaffected, and no shipped code changed on account of this. Decision 7's column, migration, port signature and call site were implemented in M4.3 as that decision said they would be; see ADR-0017's Amendments.

- **2026-08-23 (M4.4): G9 and G10 are closed, and decision 2's `check_id` caveat is resolved rather than merely corrected.** The fix is two arguments in `SemgrepAdapter.run()` — `cwd=target` with `.` as the scan target, and **`--no-rewrite-rule-ids`** — and it is recorded here rather than in ADR-0021 because it closes a caveat this document owns and introduces no new principle. Nothing in decision 2 or decision 3 changes: `rule_id` is still the tool's own stable identifier normalized by the mapper, and `compute_dedup_hash` still takes `source` as data.
  - **Measured across four CWD/argument combinations against semgrep 1.161.0**, not deduced, and the table is recorded because it is what makes the choice reviewable:

    | # | CWD | target | `check_id` | `path` |
    |---|---|---|---|---|
    | 1 | repo root | abs file | `src.verion.…rulesets.dangerous-eval` | absolute |
    | 2 | target dir | `vulnerable.py` | `C.Users.…Verion.src.…dangerous-eval` | relative |
    | 3 | `/` | abs file | `C.Users.…Verion.src.…dangerous-eval` | absolute |
    | 4 | target dir, `--no-rewrite-rule-ids` | `vulnerable.py` | **`dangerous-eval`** | **relative** |

    Row 1 is production before M4.4. Row 2 is G9's own prescribed fix and reproduces G10's prediction exactly — moving CWD to a `mkdtemp` puts the ruleset outside CWD and flips `check_id` to the absolute dotted install path, so fixing `file_path` re-keys every finding through `rule_id` in the same change. Row 4 is the resolution, and it is also what **decouples** the two: once the path is out of `check_id` entirely, `cwd` governs `path` alone.
  - **`--rewrite-rule-ids` is semgrep's own documented switch for this** ("Rewrite rule ids when they appear in nested sub-directories"), on by default. Negating it makes `check_id` the rule's own `id` from `rulesets/default.yml`. Namespacing therefore stays available and becomes *ours to author* rather than a function of where the file sits.
  - **G10's proposed mitigation — pinning the ruleset to a fixed absolute path in the container image — is dropped rather than deferred**, and the reason is the one G9 and G8 both record. It makes the absolute dotted form stable *within one deployment* and nowhere else: a developer's machine, CI and the container each produce a different `rule_id`, so the two contexts that verify this code produce a value production does not write. It also leaves an absolute worker filesystem path inside a `Finding` field that M4.5 returns in a response body. The flag makes the value independent of deployment configuration altogether. Pinning the path remains good practice; it is no longer load-bearing for identity.
  - **What proves it is not a fixture**, by G10's own measurement — a committed capture records one machine's CWD. `test_semgrep_adapter.py` runs the real adapter from two different process CWDs and asserts the two `check_id` values are **equal**, which is the invariant rather than a value. Mutation-tested, and the result corrected an assumption worth recording: **that test does not catch a missing `--no-rewrite-rule-ids`**, because `cwd=target` already fixes the child's CWD. Each flag is pinned by its own equality assertion (`check_id == "dangerous-eval"`, `path == "vulnerable.py"`); the invariance test catches the broader shape, an absolute target with no `cwd`. The previous assertion, `endswith("dangerous-eval")`, passed for all three of the CWD-dependent values above and so could not have caught any of it.
  - **The committed fixture was half pre-fix and half post-fix**, which is the disconnected-verification shape this entry keeps circling: `semgrep_scan.json` carried the CWD-dependent `check_id` beside a `path` already redacted to the relative form the fix produces. Both are now the production shape, recorded in that directory's README as a correction rather than a redaction.
  - **G9's adjacent Trivy question is closed too, and the existing capture already answered it.** G9's note distrusted the Trivy fixture because it was "only ever captured inside a container mount" — but the mount is what makes it *valid* for this question, since the question is whether an absolute argument is echoed into `Results[].Target` and `/target` is absolute. Re-measured against `aquasec/trivy` with a mount path deliberately shaped like `GitRepoCheckout`'s `mkdtemp` output: `ArtifactName` echoes the absolute argument verbatim, `Results[].Target` is the relative `requirements.txt`. `map_trivy_output` reads `Target`, not `ArtifactName`, so **Trivy's `file_path` was never affected and `TrivyAdapter` needs no change.**

- **2026-08-24 (M5.1): every fixture-derived figure in this document moved, because the corpus was replaced. No decision changes.** G23's capture (commit `00708b9`) replaced the three real fixtures with one common target, so the numbers below are re-derived rather than scaled. Each is a *record of a measurement*, and each is corrected in place here rather than in its own paragraph, so one entry carries the whole delta.
  - **Context: "Trivy's twelve vulnerabilities are unique on `(VulnerabilityID, PkgName)` alone"** → **twenty**, across three packages (Flask 2, Werkzeug 6, urllib3 12) rather than one. Still unique on that pair, which is the property the sentence exists for, and it is now a stronger statement: uniqueness across several packages in one manifest is what `PkgName` was in the key for, and the old corpus could not exercise it.
  - **Context: "One alert's three instances carry ids `6, 7, 5`"** → the CSP alert's **four** instances carry ids `6, 5, 1, 0`. The claim is about ZAP's crawl order being non-monotonic, which holds; only the cited data is new.
  - **Decision 2: "all 30 elements across the six committed fixtures carry their tool's identifier — 0 missing"** → **39** (26 real: Semgrep 1 `check_id`, Trivy 20 `VulnerabilityID`, ZAP 5 `alertRef`; plus 13 unchanged synthetic), still **0 missing**. **The deferral this bounds is therefore still bounded** — `"(unidentified)"` remains unreached by anything ever captured. The sentence's own escalation condition ("if a re-capture ever brings an element without one") did not fire, and this is the first re-capture to test it. **It is still guarded by nothing**, which is **G20** and is unchanged by this amendment; the incidental guard in `test_each_vulnerability_gets_its_own_identity` did improve, though, since M5.1 rewrote its expected set to derive from every `Vulnerabilities` entry instead of a hardcoded `range(12)` that covered the first twelve by position.
  - **Decision 4: "Measured cost on the real fixture: 4 findings → 11, and evidence bytes 8,523 → 18,148 (2.13×)"** → **5 alerts → 13 findings**. The byte figures are *not* re-derived here and should not be quoted from this line for the new corpus; what mattered to the decision is the ratio's shape, and ADR-0018's amortization argument — a one-time constant against an unchanged per-scan recurring term — is untouched by either capture.
  - **Decision 3's principle and every hash input are unaffected.** Note in passing that the new corpus contains the first real multi-`CweIDs` vulnerability, which is *not* a decision 3 concern precisely because `cwe` is excluded from `dedup_hash` — see ADR-0018's amendment of the same date, and **G26** for the stability question that exclusion does not cover.
