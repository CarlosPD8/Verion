...
- **Maximum CWEs per element is 1, on every tool.** Trivy emits exactly one `CweIDs` entry on all 12 vulnerabilities; ZAP's `cweid` is a scalar; Semgrep's `extra.metadata` is `{}`. *(**Falsified — the maximum is 2**; see Amendments, 2026-08-24. G23's re-capture gives Trivy **20** vulnerabilities with cardinality `{1: 19, 2: 1}`.)*
...
**The Semgrep fixture is degraded in two independent ways, and a reader comparing them will conflate them if this is not said explicitly.** The empty `metadata` (no CWE, no OWASP — G6) is a property of the *pinned ruleset*; the `"requires login"` fields (G7) are a property of the *missing token*. Neither fix touches the other. In particular, **the CWE cardinality measurement below is unaffected by the token**: `metadata` is not token-gated, so Semgrep's contribution to that measurement would be zero CWEs with or without one, and the maximum of 1 comes from Trivy and ZAP. *(**The maximum is 2, from Trivy** — see Amendments, 2026-08-24. The clause about the token is unaffected and is what this sentence is for: `metadata` is not token-gated either way.)* Setting a token would not change the decision in section 4; widening the ruleset could, which is why G6 names re-reading it as the trigger.
...
**`cwe` is a single `str | None`, and that is measured rather than assumed.** The maximum across every element of all three real fixtures is 1. *(**Falsified — the measured maximum is now 2**, and the decision nonetheless stands; see Amendments, 2026-08-24. The sentence is left as written because the amendment is about how it was arrived at, and rewriting it here would hide that.)* An earlier draft reached the same shape by taking the first of a list, on the reasoning that a list makes "same CWE" ambiguous for correlation — that reasoning was wrong and is recorded here so it is not repeated: set correlation is well defined (non-empty intersection) and strictly more expressive than equality, so taking the first would not remove ambiguity, it would manufacture silent false negatives in M5 whenever the shared CWE was not first.

**What the measurement is and is not evidence for, since a single number invites over-reading.** It is a fact about *these three targets* — one `requirements.txt`, one Python file, one static HTML page — not a fact about the tools. *(Those three targets were replaced at M5.1 by one common target, and the maximum became 2 — see Amendments, 2026-08-24. This sentence's argument is what survived the change and predicted it.)* Other ecosystems demonstrably emit several CweIDs per advisory. So the decision is "one value is right for the data this project has seen, and widening is cheap if that changes", not "these tools emit one CWE". Two tests carry that distinction because one cannot: **`test_the_real_cwe_cardinality_is_one_apart_from_a_single_two` guards the fixture** (named `test_every_real_vulnerability_carries_exactly_one_cwe` until M5.1, when the measurement it asserts was falsified — see Amendments) — the assertion is a literal and the fixture is data read at run time, so a re-capture bringing two fails it (verified by mutation, not assumed) — while **`test_multiple_cwes_keep_the_first_and_lose_none` guards the mapper's behaviour** against a synthetic two-CWE vulnerability, so the truncation is specified rather than incidental. Without the second, `next(iter(...))` would be an unexamined silent drop and the first production npm scan would be where anyone found out. Nothing is discarded either way: the full list stays in `raw_payload`, so M5 can widen to set intersection later without a re-scan.
...
- **2026-08-24 (M5.1): decision 2's criterion gains a measured scope limit. The criterion is not changed and is not overturned — it is incomplete, and this records where.** It says structures "stay with the module that owns them and travel by indirect import". M5.1 measured that **indirect import reaches only `application/`**: the mechanism needs a call site whose type comes back from a port's return annotation, and a `domain/`-layer consumer has no such site — it has only its own signature, which under `mypy --strict` must name the type. `layers-correlation` then forbids `correlation/domain/` from reaching even `correlation`'s own ports, so there is no route by which a structure can travel into another module's `domain/` at all. Both halves were probed rather than deduced; see ADR-0023's Context for the verbatim `lint-imports` output, and note that `correlation/` had no code, so those were the first-ever exercise of either contract.
  - **Every instance of the precedent in `src/` today is application-layer**, which is why the gap was invisible: `NormalizeScanUseCase` is the worked example this criterion is usually cited through, and it lives in `normalization/application/`. It also does less than the criterion is read as promising — it destructures `ScanResult` to `str` at the boundary and passes no entity into any annotated signature. Reading attributes off a port-returned value is demonstrated; passing the entity is not.
  - **Read the criterion as: structures stay with their owner and travel by indirect import *to an application-layer consumer*.** A `domain/`-layer consumer in another module needs something else — ADR-0023 chooses a match key that consumer owns.
  - **M6, M7 and M8 each hit this same wall**, since `risk_engine`, `brief` and `history` all read another module's entities. Recorded here rather than only in ADR-0023 because this document is where the criterion lives, and a scope limit kept only in the ADR that discovered it is a limit the next reader will not find.
  - Decision 2's `shared_kernel/` half is **unaffected and was re-applied in the negative for a fourth time**: ADR-0023 rejects both promoting `Finding` to `shared_kernel/` and putting a structural `Protocol` there. The second rejection is recorded as likely to be re-proposed at M6, when three more modules need the same thing and the one-copy-instead-of-four argument stops being hypothetical.

- **2026-08-24 (M5.1): decision 4's CWE-cardinality measurement is FALSIFIED. The decision it supports is unaffected.** This is a different category from the two amendments above and the distinction is the point of writing it out: decision 5 was *discharged*, decision 2's criterion was *incomplete*, and this one is **wrong** — a stated measurement that a later measurement contradicts.
  - **What was claimed:** *"The maximum across every element of all three real fixtures is 1."*
  - **What is now measured:** the maximum is **2**. G23's common-target capture (commit `00708b9`) gives Trivy 20 vulnerabilities with cardinality `{1: 19, 2: 1}` — `CVE-2024-49767` (Werkzeug 2.3.8) carries `["CWE-400", "CWE-770"]`. Semgrep still contributes 0 (G6) and ZAP is structurally 1, `cweid` being scalar.
  - **What stands, and why this is not a reversal.** `Finding.cwe` remains a single `str | None`. The reasoning that actually load-bears was never the number: decision 4 states in the next paragraph that the measurement "is a fact about *these three targets* … not a fact about the tools", that "other ecosystems demonstrably emit several CweIDs per advisory", and that "nothing is discarded either way: the full list stays in `raw_payload`, so M5 can widen to set intersection later without a re-scan". All three sentences are still true, and the second anticipated exactly this event. The decision was correctly hedged; only its headline number was overstated.
  - **The guard worked, which is the part worth recording.** Decision 4 named `test_every_real_vulnerability_carries_exactly_one_cwe` as guarding the fixture "verified by mutation, not assumed", and it is what made the falsification loud: it failed on the re-capture rather than letting the ADR go quietly stale. That test is **renamed** to `test_the_real_cwe_cardinality_is_one_apart_from_a_single_two`, because its old name asserted the falsified claim in the one place a reader trusts without opening the body, and it now pins the new cardinality *and* the identity of the two-CWE vulnerability. Its sibling `test_multiple_cwes_keep_the_first_and_lose_none` is unchanged and needed no change — it specified the mapper's behaviour on multi-CWE input before any real element exercised it, which is why nothing in production had to change when one arrived. Contrast the identifier measurement in ADR-0019 decision 2, which has no such guard and is tracked as **G20**.
  - **One property this event did NOT establish, and a new gap.** The mapper keeps the first `CweIDs` entry in document order, but in the only real case `CWE-400` is also the lower number, so the corpus cannot distinguish "first" from "lowest" — only `mappers/trivy.py` does. More importantly, decision 4 assumed the *discarded* CWEs were discardable; it never established that the *survivor* is deterministic **over time**. Trivy's `CweIDs` ordering across vulnerability-DB updates is unmeasured, and `cwe` is in the Postgres upsert's refresh set, so a reordering would rewrite stored rows with nothing recording it. Opened as **G26**, with a cheap exit condition: map one fixture against two DB versions and compare.
  - **The Alternatives entry `cwes: tuple[str, ...]` is now live rather than hypothetical.** It says the tuple "is the right answer if the measurement changes". The measurement has changed. It is still not adopted — a maximum of 2 on one element does not yet pay for set-intersection semantics across M5–M8 — but the trigger it named has fired, so a future proposal is a re-read of a live question rather than reopening a settled one. That entry's "the measured maximum is 1 on every tool today" is corrected by this amendment.
  - Decisions 1, 2, 3, 5 and 6 are unaffected.

- **2026-09-16 (M6.1): decision 5's `confidence` deferral is DISCHARGED as a decline, and the two questions decisions 1 and 3 left to M6 are answered.** ADR-0005 is where all three land; nothing here is reversed.
  - **`confidence` is declined, not adopted.** Decision 5's two grounds held up, and ADR-0005 adds a third this document could not have: **the degrees-versus-states hazard is not exhibited by either committed capture.** Only codes `1`, `2` and `3` occur (passive `{3: 8, 2: 5}`, active `{3: 9, 2: 6, 1: 1}`); no `Confirmed` and no `False Positive` appears anywhere. So the scale decision 5 declined to guess would still be a guess — designed against ZAP's documentation rather than against data, which is precisely the failure this ADR's own Context records, where a documented `High`/`Medium`/`Low` severity field turned out not to exist in the report. `tests/unit/test_finding.py`'s `assert not hasattr(_finding(), "confidence")` therefore stands unchanged. Re-propose on a capture exhibiting a state value, or on a second tool supplying confidence.
  - **Decision 1's question — what `UNKNOWN` means to a score — is answered explicitly**, as this document insisted it must be: `UNKNOWN` contributes nothing to the score and is never the maximum, so the enum's bottom rank stays a sort position and never becomes a judgment. Measured caveat: `UNKNOWN` is **0 of 34 and 0 of 37** in the corpora, so that rule is unreachable from committed data and M6.2 owes it a synthetic fixture.
  - **Decision 3's question — whether `source` compensates for the relative-position loss — is answered yes.** `Finding.source` is a scoring input twice over in ADR-0005 decision 1 (the exposure term and the corroboration term). `native_severity` is deliberately *not* an input: comparing `ERROR` against `HIGH` across tools is the incomparable-scales problem decision 1 exists to prevent, so the literal value stays provenance, exactly as decision 3 designed it.
  - Decision 2's `shared_kernel/` criterion is **re-applied in the negative for a fifth time**: ADR-0005 decision 2 rejects ADR-0023's alternative 6, the structural `Protocol` in `shared_kernel/` that alternative predicted would be re-proposed at M6. Decisions 4 and 6 are unaffected.

- **2026-09-20 (M8.5, documentation commit): decision 2's criterion admits its THIRD ENUM — the first admission since the criterion was written.** ADR-0037 decides `Confidence` — `ungrouped`/`reported`/`inferred`, a Risk's grouping provenance — into `shared_kernel/confidence.py`. `correlation` produces the value and `risk_engine` compares it; neither may import the other's `domain/` (rule 3). That is the criterion as written: a closed vocabulary two modules must compare, not merely transport. **This commit writes no `src/`**; the file lands in M8.5's second commit.
  - **The first admission SINCE the criterion was written, which is not the same as the first positive application — and the difference is this decision's own record.** Decision 2 admitted `Severity` in the very act of stating the criterion, and its paragraph says so: *"Applied three times in this issue, twice in the negative: `Severity` **in** (decision 2), `Confidence` out (decision 5), `Location` out."* Every application **since** has been a refusal, five of them by this section's own occasion-counting — `Confidence` and `Location` (this issue), `dedup_hash` (ADR-0019 decision 6), ADR-0023 rejecting both `Finding` and a structural `Protocol` as one occasion, and ADR-0005 decision 2 rejecting that `Protocol` again. So the sentence *"without one, the third arrives by habit"* is answered: the third did not arrive by habit, and it is the first thing the criterion has let in since it acquired one.
  - **`G67` does not apply and is named so the line is visibly drawn rather than crossed.** Its subject is a **transported** structure — `NormalizationRun`'s six fields, an entity. This is compared vocabulary.
  - **The package's contents, stated accurately.** `shared_kernel/` also holds `ports.py` — `ClockPort` and `IdGeneratorPort`, two `Protocol`s that are neither closed vocabularies nor enumerations, and which therefore sit **outside** this decision's criterion. Decision 2 governs which *vocabularies* the package takes; it has never described the package's whole contents. Recorded rather than changed, because an ADR arguing from this criterion must not mis-state what it governs — and because one document said the **contrary**: `ARCHITECTURE.md` §7 listed `ClockPort`/`IdGeneratorPort` among this criterion's applications until M8.5 corrected it.
  - **Decision 2's persistence asymmetry applies, and is honoured rather than rediscovered.** *"A severity crossing a persistence or HTTP boundary must be reconstructed as `Severity(...)` before it is compared."* `security_briefs.confidence` is to be a `String` column, so `brief`'s repository reconstructs `Confidence(...)` on read rather than comparing the raw value; ADR-0037 decision 8 says so and M8.5's code commit pins it with a test.
  - **Decisions 1, 3, 4, 5 and 6 are unaffected**, and `Finding.confidence` stays declined — that is ADR-0005 decision 4's, re-affirmed in ADR-0037 decision 1 with its re-propose condition still unmet (**G94**).
...
**`cwes: tuple[str, ...]`.** Not rejected on principle, and it is the right answer if the measurement changes; the measured maximum is 1 on every tool today, and `ARCHITECTURE.md` §4 says one. The test that pins the measurement is what makes revisiting this a deliberate act. *(**Corrected 2026-08-24 (M5.1):** the maximum is now **2** — one Trivy vulnerability in the G23 corpus. The trigger this entry names has therefore fired, and the tuple is a live question rather than a settled one; still not adopted, for the reason in Amendments. The test did its job, which is why anyone knows.)*
