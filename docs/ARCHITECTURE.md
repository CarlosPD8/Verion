...
MatchGroup   # M5.8 — correlation; a candidate Risk as SHIPPED
 ...
 └── member_confidence: (Confidence, ...)   # M8.5 (ADR-0037), SHIPPED: one value per member,
                                            ...
                                            # (off the route map), ungrouped (key had no signal).
...
RiskReasoning
 ...
 # M8.5 (2026-09-20, ADR-0037) CHOSE it, and it is the GROUPING reading: a Risk's
 # confidence is how its membership was established, not how sure anyone is that a
 # finding is real. It lives on ScoredSurface, not on RiskReasoning, because it is not
 # a scored signal: it is carried BESIDE priority_score and never summed into it.
 # Why not summed: every fix_now surface reachable from real scanner output contains an
 # inferred member (G64), so a term penalising inferred would close the top bucket and
 # one rewarding it would reward inference. ADR-0037 decision 7 carries the reopen
 # condition, that being a corpus-bounded fact rather than a theorem.
 # FR-7's OUTPUT half is now met and its INPUT half is not — the per-finding confidence
 # its sentence names is still declined (ADR-0005 decision 4), and G94 carries it.
 #
...
SecurityBrief   # M7.2 — brief; SHIPPED (ADR-0033). Append-only: one row per generation
 ...
 │                                  # _DECISION_VERSION does not move and every stored Brief
 │                                  # keeps reading back. None only for a Brief before M8.5.
 └── generated_at
...
- **Brief routes** (M7.2, ADR-0033; M8.6, ADR-0038) — ~~`POST /projects/{project_id}/briefs` generates and stores a Brief for an exact finding-id set.~~ *(Struck 2026-09-21, M8.6 commit 3.)* `POST /projects/{project_id}/briefs` **accepts** a request to narrate an exact finding-id set and answers **202** with a generation id; `GET /projects/{project_id}/brief-generations/{id}` polls it, reporting `pending`/`running`/`succeeded`/`failed` and, on failure, one of three `failure_kind` values chosen on what the client does. `GET /projects/{project_id}/briefs` pages the stored Briefs. All three answer 404 for every denial (**G17**), and the poll additionally requires an actor match, so a generation is readable only by the caller who asked for it.
...
**`ClockPort` and `IdGeneratorPort` are NOT applications of this criterion.** They live in `shared_kernel/ports.py` as cross-cutting `Protocol`s (M0.3, the convention in §5.2), predate ADR-0018, and are neither closed vocabularies nor enumerations. This criterion governs which **vocabularies** the package takes, not its whole contents — noted because an argument from it should not mis-state what it covers.
...
sequenceDiagram
    ...
    Brief->>Risk: explainable_risk(finding_ids) — ExplainableRiskPort, exact set (M7.2); the WORKER's call, with the stored user_id (M8.6)
...
Stages after normalization are drawn as designed, not as built — with the exceptions named below, which have grown: `brief` is M7 and how it is triggered is that milestone's decision. *(Decided 2026-09-17, M7.2, ADR-0033 decision 8: a member's `POST /projects/{project_id}/briefs` runs the three `Brief` lines ~~synchronously on the request path. There is no queue trigger, the provider call is billed per request, and the request's session stays open across it~~ (**G73**).)* *(Marked 2026-09-17, M7.3: the diagram now draws five `Brief` lines, with member reads and two billed calls, `describe` then `explain`, ~~all on the same request path and inside the same session~~ (ADR-0034 decision 3).)* ***(Struck and replaced 2026-09-21, M8.6 commit 3, ADR-0038: **every `Brief` line above now runs in the WORKER**, under `platform/worker.py`'s `generate_brief`. The POST writes a `brief_generations` row, answers **202**, and enqueues after the commit; `GET /projects/{project_id}/brief-generations/{id}` reports the outcome. The provider call is still billed per generation — that clause was right and is untouched — but no request's session is open across it, which is the half of **G73** that resolved. The five lines do still share ONE session and one transaction, now the job's. The `Brief->>Risk` line's 404 is also gone: the exact-set refusal is a `surface_changed` generation reported by the poll (decision 6). **This whole paragraph was missed by ADR-0038's owed list and by this commit's own grep pass; the guardian found it.**)*** *(Marked 2026-09-15: M5 decided correlation's, and it has none — `ListProjectRisksUseCase` recomputes groups per request on the API path, not in the worker chain. The diagram does not draw its `ServingDeclarationPort` and `RouteMapPort` reads, or the route-map write at Security Context build.)* M5 inherits the handoff pattern above as a precedent, not as a constraint (ADR-0017).
...
*(The Workers box read scan/correlate/risk/brief until 2026-09-15, then scan/normalize until 2026-09-21. `platform/worker.py`'s `WorkerSettings` runs `run_scan`, `normalize_scan`, **`generate_brief`** and the normalization sweep. ~~correlation and scoring run on the API request path (ADR-0025, ADR-0030), and briefs are not built~~ *(struck 2026-09-21, M8.6 commit 3, ADR-0038: briefs are built here now, and `generate_brief` runs correlation and scoring in the worker to reach the surface it narrates — ADR-0005 decision 3 still has that use case persist nothing, so each job recomputes the project's whole scored set (**G61**). Correlation and scoring still ALSO run on the request path, for `/scored-risks` and the findings routes; what is false is "not built" and the word "the", which said the request path was the only one.)* **The worker process reads `OPENAI_API_KEY` from this commit on** — new deployment surface, not a correction: no document said only the API read it (ADR-0032's 2026-09-21 amendment).)*
...
- **ADR-0033 — How a Security Brief refers to its Risk, what it stores, how it is read, and who may generate one.** M7.2. **G68's fork is taken without giving a Risk an identifier.**
  - **Identity and address.** A Brief has its own surrogate id and holds its Risk's ordered finding-id set as **data, not as an address**. The set acts as a selector, resolved by exact equality and failing closed when membership has changed. ~~once, in `POST /projects/{project_id}/briefs`, resolved in the same request … with a 404~~ *(struck 2026-09-21, M8.6 commit 3, ADR-0038 decision 1: the resolution moved to the worker, so the POST answers 202 and the failure arrives at the poll as `surface_changed`. The selector, the exact equality and the fail-closed are unchanged — only where and when. **This site is not on ADR-0038's owed list**; it was found by grepping the struck claim across the tree rather than at the sites the ADR named.)* ADR-0025 decision 1 is untouched, and decision 2's clause *"the first values about a Risk that cannot be recomputed"* is struck and replaced in that ADR's Amendments, because a stored narrative is a counterexample.
...
- **ADR-0037 — A Risk's confidence: grouping provenance, where it is computed, and what it may not claim.** M8.5, written before its code, and **shipped**. Closes a gap four issues old across two milestones: ADR-0005 deferred the scale at M6.1, and M6.2, M7.1 and M7.2 each recorded the absence rather than choosing, because — in **G63**'s words — *"either scale reaches a third module."*
  ...
  - **Never summed into the priority.** Every `fix_now` surface reachable from real scanner output contains an inferred member (**G64**), so a term penalising `inferred` would close the top bucket and one rewarding it would reward inference. That ground is corpus-bounded, so the decision carries its own reopen condition — one clause inherited from G64, two new, the strongest being a mapper change that gives a Semgrep finding a `Location.url`.
