...
| `RouteMapPort` | A project's Flask route map. `correlation` reads it to derive a route path for a finding with no signal, taking the `RouteMap` by inference and calling its `paths_serving` method (M5.6 commit 3, ADR-0029). **Empty in production until M5.6 commit 4**, which populates it at Security Context build time | `EmptyRouteMapReader`, a placeholder until commit 4 |
