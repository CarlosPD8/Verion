...
| `CorrelateFindingsUseCase` | Group related `Finding`s into candidate Risks. **Its scope — a project's findings or one scan's — is M5.8's decision and is deliberately not stated here.** *(This clause said **M5.1's** until 2026-08-25. Falsified: M5.1 closed without taking the decision and without building the use case — `ROADMAP.md` **G30** — so the decision moves with the work to **M5.8**, where it is criterion (a). Rewritten inline and dated, on the precedent this row already sets for itself in the next sentence. That sentence's own "M5.1" is left as written and reads M5.8 too; the cost it states is what makes M5.8's criterion checkable and is why the rest of this row is untouched.)* This row said "over a scan's findings" until the M4→M5 boundary review, which is a query `FindingRepositoryPort` deliberately does not offer: `scan_id` is not the leading column of the sightings primary key, and the scan-first read, its index, the absence check and the succeeded-tools caveat are M9.1's acceptance criteria, together. §8 drew the same stage as an unscoped `read Findings`, so the two sections disagreed and this one was the more specific. M5.1 choosing scan scoping means building M9.1's query first, not inheriting it |
