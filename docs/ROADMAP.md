...
- **M4.5 — Findings read API**
  ...
  - **Ships `ix_finding_sightings_scan_id`, in the migration that carries the query wanting it** (ADR-0020). M4.3 deliberately shipped no secondary index: every write path it and M4.4 use is served by a constraint index, and project-scoped listing rides the leading column of `uq_findings_project_id_dedup_hash` as an index prefix. The one query nothing serves is the scan-first one — "which findings were sighted in scan N", which is this issue's "still open" filter and M9.1's absence check — and the sighting's primary key is `(finding_id, scan_id)`, so `scan_id` is not its leading column. Additive, and cheap either side of persistence, because an index rewrites no data.
  - **`EXPLAIN` the project listing and the sighting join at realistic volume before adding anything further** (ADR-0020). No measurement was possible at M4.3: the three committed fixtures produce 24 findings for one scan, and Postgres seq-scans at that size whatever indexes exist, so a benchmark then would have measured nothing while reading as evidence. This is also the point to re-read ADR-0019 decision 1's standing invitation to denormalize a `last_seen` cache **if the join measures as the bottleneck** — it is additive, and it was refused at M4.2 as a guess, not on principle.
  - **A findings list is project-scoped and scan-independent now** (ADR-0019). "Findings from scan N" is a query over `finding_sightings`, never a `WHERE scan_id = …` on the findings table, and "still open" means sighted in the latest scan — with the succeeded-tools caveat ADR-0019's Consequences states for M9.1, which applies to any such filter here too.
  - **One pre-existing defect surfaces here**: `raw_payload[:MAX_RAW_PAYLOAD_CHARS]` truncates into invalid JSON in all three mappers, so a response schema has to decide what to do with a payload that no longer parses (a marker, or truncating at an element boundary). ADR-0019's per-instance ZAP split makes it *less* likely to fire, not more, which is why it was left rather than fixed there.
...
- **M9.1 — Re-scan diffing**
  ...
  - **The absence check needs `ix_finding_sightings_scan_id` if M4.5 has not already shipped it** (ADR-0020). The sighting's primary key is `(finding_id, scan_id)`, which serves finding-first lookups and not the scan-first one this issue is built on.
  - **Read ADR-0019's Consequences before designing the absence check.** "Not sighted in the latest scan" only means resolved for the tools that *succeeded* in that scan; scoping by scan membership alone would let one failed Trivy silently resolve every dependency finding in the project. The reasoning is in the ADR, where it stays read; this is the pointer.
