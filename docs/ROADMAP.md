...
- **M4.4 — Close the scan→normalize handoff**
  ...
  - **Prerequisite: G9.** Semgrep's `path` is an absolute per-scan temp path in production, and `file_path` is a `dedup_hash` input — so until `SemgrepAdapter` runs with `cwd=target` and passes `.`, every Semgrep finding re-keys on every scan and this issue's own end-to-end test would pass while dedup silently failed for one tool of three. Confirm the adjacent unknown while there: `trivy fs` was only ever captured inside a container mount, so whether it keeps `Results[].Target` relative against an absolute host path is unverified.
...
Confirmed: M4.2 · Status: assigned → M4.4
...
Confirmed: M4.3 · Status: assigned → M4.4
