...
- **M4.0 — Pipeline boundary: what triggers normalization, and what `Scan.status` means once it exists** — done
  ...
  - **Option — lazily on read.** Makes normalization latency user-visible at `GET /projects/{id}/findings` (M4.3), and sits badly with the milestones either side: M4.2's dedup ("re-running a scan doesn't duplicate identical findings") needs findings that persist across scans, and §8 has M5 reading `Findings` from the database, not from a request-scoped computation.
  ...
  - **Deliberately not shipped, trigger M4.1:** the arq enqueue, a `normalize_scan` job, the reconciliation sweep query, and the partial index the sweep will want. With no `Finding` entity they would be dead-by-construction. Between M4.0 and M4.1 the system writes a `pending` row on every scan that nothing advances — intended, and recorded in ADR-0017's Consequences.
...
- **M4.1 — Finding domain model**
  Module: `normalization` · Depends on: M4.0
  - `Finding`, `Evidence` entities per `ARCHITECTURE.md` §4.
  - **Capture real Semgrep/Trivy/ZAP output as fixtures before writing the mapping functions.** None exist today — the only JSON under `tests/` is GitHub API responses from M2.2 — while the criterion below and `ARCHITECTURE.md` §9's "recorded/replayed scanner output for CI stability" both assume they do. This is a prerequisite, not a parallel task: a mapper cannot be written against output nobody has looked at. Cheap now, which it was not before — M3.7's `test_multi_scanner_dispatch.py` runs all three real scanners over one scan, so a single run yields the whole set.
  ...
  - **Integration test: a `PARTIAL` scan is normalized, produces findings from the succeeded tools only, and the failed tool contributes none** — asserted through `get_succeeded_by_scan_id`, the query ADR-0017 fixed as the entry point, not by row counts alone. *Moved here from M4.0, which could not satisfy it: producing findings needs the `Finding` entity and the per-scanner mappers, and M4.0 explicitly deferred both to this issue. M4.0 asserts the handoff state instead.*
  - **Also inherited from M4.0** (see ADR-0017's Consequences, where each is recorded with its reason): the arq enqueue and a `normalize_scan` job; the reconciliation sweep, which **must select on `normalization_runs` and `scan_results` and never read `Scan.status`**; the partial index on `WHERE status = 'pending'` that the sweep query wants, deliberately not added ahead of the query it serves; and the `started_at`/`finished_at` state machine, left unconstrained in both the domain guard and the `CHECK` because those transitions are this issue's to define.
