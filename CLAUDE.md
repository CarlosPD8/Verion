...
- **Full-suite runtime is a tracked number with a threshold, measured in CI** — specifically the duration of the `Tests (pytest …)` step, **not** total job time, which also carries image pulls and tool installs and is noisier (the ZAP pull alone has ranged 26–37s). Measure in CI, not locally: local runtime varies by OS and by which tool binaries are installed, and the CI step is currently *faster* than a Windows local run despite running more tests. Baseline **136s** (2026-09-08, M5.9 — CI run `34265788717`, commit `a745975`, named so it stays re-checkable), having grown 14s → 15s → 44s → 45s → 64s → 71s → **136s** across M3.3 → M3.4 → M3.6 → 2026-08-20 → M3.7 → M4.5 → M5.9. *(The previous baseline was **71s**, 2026-08-24, M4.5 — CI run `32715167778`, commit `7cca54b`.)* **Why it moves, when the three readings before it did not:** the tracked step read **72s** at `ccc73f4` (M5.4, CI run `34151970614`), **72s** at `8af1ef7` (ADR-0027, CI run `34251436355`) and **73s** at `25488a6` (M5.9 commit 1, CI run `34258943559`), all three inside the ±8s band recorded below and therefore drift rather than growth; **136s** is a change in what the suite contains, and the 63s between 73s and 136s is attributable to `tests/integration/test_active_scan_finds_the_sink.py`, the only test the intervening commit added. It is **one reading, dated and named, exactly as 71s was** — not a settled average. **Treat it as approximate:** consecutive runs have varied by roughly ±8s, because the step contains network-bound work. Compare against the trend, not a single reading — M4's own five runs read 64s, 59s, 68s, 60s, 71s across M4.1–M4.5, which is the variance and the growth in one series — **that band has NOT been re-derived at this scale**, those five being readings of a step that contained no active scan.
