...
> **Status:** in development — M0–M3 complete, M4 (Normalization) in progress: M4.0 through M4.3 have landed, M4.4 is next. The scanning pipeline works end to end: a trigger (API or GitHub webhook) fans out to Semgrep, Trivy and OWASP ZAP concurrently and persists each tool's raw output, then records that normalization is owed for that scan. The common `Finding` schema, the three per-scanner mappers and the identity model are pure domain code tested against captured real scanner output, and the tables behind them now exist — a finding is durable and project-scoped, deduplicated on a content hash, with one sighting per scan that observes it. What is not yet wired is the job that runs them: the normalization worker is M4.4 and the read API M4.5, and correlation, risk scoring, the explanation layer and the dashboard follow. See the roadmap below.
