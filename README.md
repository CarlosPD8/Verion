...
> **Status:** in development — M0–M7 complete (M5.7, a CI job split, moved to M11), M8 (Dashboard & History) in progress — M7.2 ships Brief generation, and **since M8.6 it is a background job**: `POST /projects/{id}/briefs` takes an exact finding-id set, answers **202** with a generation id, and queues the work, which resolves the set against the current scored Risks in the worker; `GET /projects/{id}/brief-generations/{id}` reports whether it is pending, running, succeeded or failed, and on failure says whether to re-read the scored Risks, retry, or not retry. The worker narrates through M7.1's OpenAI adapter and stores the result; `GET /projects/{id}/briefs` pages the stored Briefs. M7.3 adds each Brief's *what happened*, narrated by a separate call from members' sanitized typed titles and locations, so the call that narrates priority still sees nothing scanned, and it takes the capture that was owed: a 200 per prompt and a redacted 401 from the real API are committed and replayed by the adapter's own tests, which ends the M7.1 departure. All three of M7's issues are done and its boundary review has closed it: a Brief carried three of FR-8's four parts; **M8.5 adds the fourth**, so a Brief now says what happened, why it matters, which findings are its evidence, and how confident the grouping is. Recommended action and estimated effort are cut to V2. **The pipeline runs end to end and its output is now readable over HTTP.** A scan starts from a signed GitHub push webhook or, since M8.8, when the project's owner calls `POST /projects/{id}/scans`; `GET /projects/{id}/scans/{scan_id}` reports the scan's status. It fans out to Semgrep, Trivy and OWASP ZAP concurrently, persists each tool's raw output, and records in the same transaction that normalization is owed; a separate worker job then turns the trustworthy output into `Finding` rows and records one sighting per scan that observes each one. A finding is durable and project-scoped, deduplicated on a content hash, so re-running a scan refreshes rows rather than duplicating them — and a reconciliation sweep guarantees no scan's normalization is lost even if the queue drops the message. `GET /projects/{id}/findings` returns a project's findings most-severe-first with each one's sighting history, and says whether normalization actually completed, so a short list caused by a failed pipeline stage is distinguishable from a clean project. Raw tool output is a separate, per-finding route rather than a field on the listing, because it is a verbatim copy of scanned source. `GET /projects/{id}/risks` groups a project's findings into candidate Risks on shared signals, and for a Flask project whose owner has declared that the scanned URL serves the connected repository, a Semgrep finding and a ZAP finding on the same route land in one group. `GET /projects/{id}/scored-risks` returns the same groups ranked, each carrying the signals that produced its priority, computed per request and persisting nothing; each item also says how its membership was established — `reported` when every finding was placed there by a field its own scanner reported, `inferred` when at least one was placed by Verion matching a file and line against the project's route map, and `ungrouped` when the finding carried no signal to group on at all, which is what every Semgrep finding gets on a project with no serving declaration in force — with the envelope defining all three, since M8.5. Since M8.1 any member can dismiss a scored Risk with a reason, `POST /projects/{id}/risk-dismissals`, and undo it; each dismissal is an append-only record listed with its actor at `GET /projects/{id}/risk-dismissals`, and a Risk a new finding joins is no longer dismissed. The ranked list does not show dismissals yet (M8.2). A first two-page frontend (M8.3, started early) signs in and renders that ranked list with each priority's working shown. The dashboard follows. See the roadmap below.
